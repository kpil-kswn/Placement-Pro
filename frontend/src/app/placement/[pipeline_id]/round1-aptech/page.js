...
export default function AptechRoundPage() {
  ...
  const handleSubmit = useCallback(async (isAutoSubmit = false) => {
    if (!isAutoSubmit) {
      const confirmSubmit = confirm("Are you sure you want to submit? You cannot return to this round.");
  ...
  if (questions.length === 0) {
    return <div className="min-h-screen flex items-center justify-center font-bold text-gray-500">Loading Assessment...</div>;
  }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center sticky top-0 z-10 shadow-sm">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Round 1: Aptitude & Technical</h1>
          <p className="text-sm text-gray-500">Question {currentIndex + 1} of {questions.length}</p>
        </div>
        <div className={`text-xl font-mono font-bold px-4 py-2 rounded-lg border ${timeLeft < 300 ? 'bg-red-50 text-red-600 border-red-200 animate-pulse' : 'bg-blue-50 text-blue-700 border-blue-200'}`}>
          ⏱ {formatTime(timeLeft)}
        </div>
      </header>
