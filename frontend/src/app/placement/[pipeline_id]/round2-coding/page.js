...
export default function CodingRoundPage() {
    ...
    const fetchProblems = async () => {
      try {
        ...
        fetchedProblems.forEach((p) => {
          initialCodes[p.problem_id] =
            p.user_code || p.problem_data?.starter_code || "";
        });
  ...
  const handleSubmitAll = async (isAutoSubmit = false) => {
    if (!isAutoSubmit) {
      if (
        !confirm(
          "Are you sure you want to submit ALL problems? You cannot return to this round.",
        )
      )
        return;
    }
    ...
    try {
      const res = await fetch(`/api/placement/${pipelineId}/submit-coding`, {
        method: "POST",
      });
      const data = await res.json();
      if (res.ok) {
    ...
    <div className="flex h-screen w-full bg-gray-50 flex-col overflow-hidden">
      ...
      <header className="bg-slate-900 text-white px-6 py-3 flex justify-between items-center shrink-0 shadow-md z-10">
        ...
        <div className="flex items-center gap-6">
          <div
            className={`font-mono text-lg font-bold px-4 py-1 rounded bg-slate-800 border ${timeLeft < 300 ? "text-red-400 border-red-500 animate-pulse" : "text-green-400 border-green-500"}`}
          >
            ⏱ {formatTime(timeLeft)}
          </div>
          <button
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-center">
                    <span
                      className={`font-bold ${activeProblemIdx === idx ? "text-blue-800" : "text-gray-700"}`}
                    >
                      Problem {idx + 1}
                    </span>
        ...
        <div className="flex-1 flex flex-col border-r border-gray-200 bg-white min-w-[350px]">
          ...
          <div className="flex border-b bg-gray-50 shrink-0">
            ...
            >
              ...
              {activeResult && (
                <span
                  className={`w-2 h-2 rounded-full ${activeResult.status === "Passed" ? "bg-green-500" : activeResult.status === "Error" ? "bg-yellow-500" : "bg-red-500"}`}
                ></span>
              )}
            </button>
          </div>
            ...
            {activeTab === "description" && (
              <div className="prose prose-sm max-w-none text-gray-800">
                <h2 className="text-xl font-bold mb-4">
                  {activeProblem.problem_data?.title ||
                    `Problem ${activeProblemIdx + 1}`}
                </h2>
                <ReactMarkdown>
                  {activeProblem.problem_data?.problem_statement ||
                    "Description loading..."}
                </ReactMarkdown>
              </div>
            )}
        ...
        <div className="flex-1 flex flex-col bg-[#1e1e1e] min-w-[400px]">
          <div className="flex justify-between items-center p-2 bg-slate-900 border-b border-gray-700 shrink-0">
            <select
              ...
              className="bg-slate-800 text-white border border-gray-600 rounded px-3 py-1 text-sm outline-none font-semibold"
            >
